...
switch_ROI = .5  # Do you want to switch the ROIs over part way through and if so, specify the proportion of TRs before this happens
...
#Instead of a coinflip, interleave trials

onsets = []
for event_counter in range(events):
    onsets.append(event_counter * event_duration)

onsets_A = onsets[0:][::4] + onsets[1:][::4]
onsets_B = onsets[2:][::4] + onsets[3:][::4]
